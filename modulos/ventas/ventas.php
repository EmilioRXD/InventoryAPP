...
function hacer_venta($productos, $total, $total_bs, $precio_verde, $metodo_pago, $ticket, $cambio)
{
    ...
    foreach ($productos as $producto) {
        ...
        $sentencia = $base_de_datos->prepare("INSERT INTO ventas(numero_venta, codigo_producto, nombre_producto, total, total_bs, precio_venta, precio_verde, fecha, numero_productos, metodo_pago, usuario, familia, utilidad) VALUES (?,?,?,?,?,?,?,?,?,?,?, ?, ?);");
        $resultado_sentencia = $sentencia->execute(array($numero_venta, $producto->codigo, $producto->nombre, $producto->cantidad * $producto->precio_venta, $total_bs, $producto->precio_venta, $precio_verde, date("Y-m-d H:i:s"), $producto->cantidad, $metodo_pago, $_SESSION["nombre_de_usuario"], $producto->familia, $producto->utilidad * $producto->cantidad));
...
function hacer_credito($nombre_cliente, $numero_cliente, $productos, $total, $metodo_pago, $ticket, $cambio)
{
    ...
    foreach ($productos as $producto) {
        $todo_correcto = $todo_correcto and quitar_piezas($producto->cantidad, $producto->rowid);
        $sentencia = $base_de_datos->prepare("INSERT INTO creditos(delet, numero_credito, nombre_cliente, numero_cliente, codigo_producto, nombre_producto, total, totalR, fecha, numero_productos, metodo_pago, usuario, familia, utilidad) VALUES (?, ?, ?, ?, ?, ?,?,?,?, ?, ?, ?, ?, ?);");
        $resultado_sentencia = $sentencia->execute(array($delet, $numero_credito, $nombre_cliente, $numero_cliente, $producto->codigo, $producto->nombre, $total, $total, date("Y-m-d H:i:s"), $producto->cantidad, $metodo_pago, $_SESSION["nombre_de_usuario"],$producto->familia, $producto->utilidad * $producto->cantidad));
        $todo_correcto = $todo_correcto and $resultado_sentencia;
    }
    $todo_correcto = $todo_correcto and ingresar_dinero_venta_caja($total, $numero_venta);
    if ($ticket === TRUE) {
